# ONYVEL Brand Archive

Public timestamped archive for the **ONYVEL** brand.

This repository records public evidence of ONYVEL's brand identity, website presence, campaign language and first commercial product as they existed on **21 August 2026**.

## Official website

- https://onyvel.com/

## Initial public brand elements

- Brand: **ONYVEL**
- Collection: **DROP 001**
- Campaign phrase: **REALITY WAS OVERRATED.**
- Secondary campaign phrase: **HUMAN OPTIONAL.**
- Brand line: **Designed to be desired.**
- First product: **ONYVEL / DROP 001 — Reality Was Overrated Oversized Tee**

## Purpose

The purpose of this repository is archival and evidentiary. It provides a public, version-controlled record showing the existence and public use of the ONYVEL brand and associated creative material at a specific point in time.

Git history and SHA-256 checksums provide additional integrity evidence for archived documents and assets.

## Archive structure

- `evidence/2026-08-21/` — initial public evidence snapshot
- `assets/` — selected public brand assets
- `documents/` — timestamped brand evidence documents
- `scripts/` — checksum helpers
- `CHECKSUMS.sha256` — SHA-256 hashes for repository evidence files

## External snapshots

The public URLs listed in `evidence/2026-08-21/archive-links.md` are intended to also be preserved through independent web-archiving services such as the Internet Archive Wayback Machine.

## Rights

All ONYVEL names, logos, campaign wording, artwork, product designs and related creative material remain the property of their respective rights holder(s). Publication in this repository does **not** grant a license to reproduce, redistribute or commercially exploit the material.

This repository is an archive, not an open-source release.
